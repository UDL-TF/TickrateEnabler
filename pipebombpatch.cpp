...
// Patch Failure: Couldn't find CTFWeaponBaseGrenadeProj::VPhysicsUpdate() in server memory.
...
int PipeBombFrameTimePatch::FrameTimeNumCounter = 0;
...
#elif defined(_WIN32)
	if (len > 0)
		return (BYTE *)g_MemUtils.FindLibPattern(gamedll, sig, len);
	else
		return nullptr;
#endif
}
